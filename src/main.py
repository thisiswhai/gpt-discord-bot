...
async def on_ready():
    logger.info(
        f"We have logged in as {client.user.name}. Invite URL: {BOT_INVITE_URL}")
...
async def chat_command(int: discord.Interaction, message: str):
    try:
        ...
        try:
            ...
            embed.add_field(name=user.display_name, value=message)
            ...
            await int.followup.send(embed=embed, ephemeral=True)
        ...
        thread = await response.create_thread(
            name=f"{ACTIVATE_THREAD_PREFX} {user.display_name[:20]} - {message[:30]}",
            slowmode_delay=1,
            reason="gpt-bot",
        ...
        async with thread.typing():
            # fetch completion
            messages = [get_message(user=user.display_name, text=message)]
...
async def on_message(message: DiscordMessage):
    try:
        ...
        if isinstance(channel, discord.Thread):
            # ignore messages with mentions (unless to the bot)
